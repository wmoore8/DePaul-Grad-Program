...
object fp2 {
  ...
  def joinTerminateRight (xs : List[String], term : String) : String = {
    ...
    xs match {
    	case Nil => ""
    	case x::xs => x + foldRight(xs, term, (y:String, z:String) => term + y + z)
    }
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    ...
    xs match {
    	case Nil => ""
    	case x::xs => x + foldLeft(xs, term, (y:String, z:String) => y + z + term)
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    def auxIndex (xs: List[Int], a: Int, i: Int) : Int = {
    	xs match {
    		case Nil => -1
    		case x::xs => if (x >= a) {
    			i
