...
object fp3 {
  ...
  def delete3 [X] (x:X, ys:List[X]) : List[X] = {
    // TODO: Provide definition here.
    ys.filter(z => (if (z != x) true else false))
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def removeDupesHelper [X] (count: Int, toCompare:X, ys:List[X]) : List[(Int, X)] = {
    	ys match {
    		case Nil => (count, toCompare) :: Nil

    		case y :: ys => if (y == toCompare) {
    			removeDupesHelper(count+1, y, ys)
    		} else {
    			(count, toCompare) :: removeDupesHelper(1, y, ys)
    		}
    	}
    }
    ...
    xs match {
    	case Nil => Nil
    	case x :: Nil => (1, x) :: Nil
    	case x :: xs => removeDupesHelper(1, x, xs)
    }
  }
